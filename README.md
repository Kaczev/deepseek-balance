...
| C1–C4 | 标题行、余额数字、滚动、币种 | 完成。标题兼状态行（左上）、余额数字（居中，合并量宽后整体居中）、币种符号作**后缀**（`100.00¥`，所有者指定）、显示值渐进跟随并吸附。自检 `--selftest-b` 共 28 项全过 |

### 看视觉时要按真实尺寸看

导出的是**物理像素位图**。在 200% 缩放的屏上，200% 的导出图会撑满屏幕，从而**看不出字号合不合适**。所以导帧支持指定缩放：

```
--dpi=96    按 100% 导出（475×289）—— 设计尺寸，多数屏幕上的真实观感
--dpi=144   按 150% 导出（713×434）
--dpi=192   按 200% 导出（950×578）—— 本机物理像素
```

`build\make-compare.ps1` 会把 100% 与 200% 拼成一张对比图（`build\dpi-compare.png`），先看 100% 那一块再下判断。
