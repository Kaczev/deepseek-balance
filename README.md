...
| B 阶段 | 数据管道 + 调试开关 | 完成。金额十进制解析、采样记录、模拟数据源（九种情形）、连接状态机、调试浮层。自检 `--selftest-b` 全过（21 项） |

### 现在的画面是什么

一块圆角半透明蓝色面板，中间有个白色方块在走（那是"帧循环还活着"的证据）。带 `--debug` 时左上角有一层淡黄色文字，那是**调试浮层**：它显示状态机当前判断出的连接状态、余额、当前情形、已收集样本数、有没有 Key、窗口有没有焦点。

浮层是**眼睛**，不是产品界面——正文的数字、标题、曲线、清零预估在 C/D/E 阶段做。
