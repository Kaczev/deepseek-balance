...
struct WidgetFrame {
    ...
    // 标题那一行后面的灰色小字"今日已 X.XX¥"（UTF-8；空串 = 这条数据算不出来，不画）。
    // ★ 与 amountText 同一个形状（窄串、由渲染层再转宽）：跨进程/跨模块要传的是**文本**，
    //   不是"一个金额 + 一个格式"，否则百分比、破折号这些口径会分叉成两份。
...
// ---------------------------------------------------------------------------
// "今日已 X.XX¥"（所有者 2026-09-19）：标题那一行后面那串灰色小字
// ---------------------------------------------------------------------------
//  口径（所有者定，别自己改）：
//    · **自然日**，本地时间 00:00 起算；
//    · 只累计**下降**：今天每两个相邻的可用余额点之间 (前 − 后) 里的**正**部分相加，
//      上涨（充值）只让它自己那一步贡献 0 —— 所以充值既不会把这行冲小，也不会让它变负。
//
//  ★ 为什么归显示层而不是渲染层：渲染层不知道"余额从哪来"（见 WidgetFrame 的注释），
//    而这一行的输入正是曲线存储里的一串余额。它与清零预估、曲线一样属于"显示什么"。
//
//  ★ 每个点自带 `at`（curve_store §2.3b）。没时间的点（老文件）不参与：那一步的
//    余额差是真的，但它属于哪一天没人测过 —— 不拿存储的全局 update_at 顶替
//    （与速率估计器同一条规矩）。
//  ★ 全程**整数**：Amount::raw 是 1/10000 元的整数，减法与求和都在它上面做，
//    不经过 double（设计 §3.1）。显示的两位小数由 Amount::ToString2 负责。
struct TodayUsage {
    // 单位与 Amount::raw 相同（1/10000 元），≥ 0。天数内所有可用的下降之和。
    int64_t raw = 0;
    // false = 这条数据算不出来（今天一个可用的点都没有，或者存储里找得到今天的余额、
    // 却找不到任何一次今天之前的余额）：界面显示所有者原话里那个 `--.--` 形状。
    // ★ 注意它**不是**"下降为 0"：今天只涨不跌时 ok 是 true、raw 是 0，界面写 0.00¥。
    //   两者的区别是"算出来是零"与"算不出来"，与余额那一行 `0.00` / `--.--` 同一条规矩。
    bool ok = false;
};

// 从曲线存储（当前加载的那一份）算出今天已经消耗了多少。
//  `currency` 空 = 每个点退回到它第一个有值的条目（曲线取值与速率估计都用这一条口径）。
TodayUsage TodayUsageFromStore(const std::string& currency);

// 渲染层要画的那串文本："今日已 X.XX¥" / "今日已 --.--¥"，或者**空串**。
//  空串的唯一含义是"账户欠款、没有可显示的余额"——那种状态下标题、倒计时、数字
//  都由显示层收起来了（见 BuildWidgetFrame 开头那一段），这一行没有理由还留着。
// ★ 颜色不在文本里：灰色是 renderer.cpp 的事（kEdgeTextColor*，与标题同一个色）。
std::string TodayUsageText();

// 今天零点（本地时间）的 epoch 秒。
int64_t TodayStartSeconds(int64_t nowSeconds);

// 这一层眼里的"现在"（epoch 秒）：没设夹具时就是真实墙钟。TodayUsageFromStore 用它。
int64_t TodayUsageNowSeconds();

// ★ 探针用的时钟夹具（panelprobe 的 case5）：把这一层眼里的"现在"钉在一个固定时刻。
//   不设的时候它一直是 `std::time(nullptr)`，生产路径不调它 —— 与 SetCountdownText /
//   SetLayoutProbe 同一套写法。
//   为什么需要：这一行的结果依赖"今天"，一个按真实时钟跑的命令明天会给出另一个数字，
//   而"导帧可复现"是这个项目唯一的验证方式（PrimeHistoryForDemo 的固定锚点同一个理由）。
void SetTodayUsageNowForProbe(int64_t nowSeconds);
