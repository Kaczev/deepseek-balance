...
// 数字的字号**按位数自适应**：下标 = 数字字符的个数（币种符号不算）。
//   下标:  0    1    2    3    4    5    6    7    8
//   位数:  1    2    3    4    5    6    7    8    9+
// 于是 1~4 位都是 40；位数变多时字号逐档下降，版面跟着收，
// 数字始终完整落在面板里。
// 最后一项也是"位数超出表长"时的兜底字号（见 renderer.cpp 的 sizeIdx 钳位）。
inline constexpr float kNumberSizeByDigits[] = {
    40.0f, 40.0f, 40.0f, 40.0f, 38.0f, 34.0f, 30.0f, 27.0f, 24.0f,
};

// 右上角那行："deepseek 余额"（同时兼状态文案）。
inline constexpr float kTitleSizeDip = 12.5f;
