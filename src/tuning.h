...
// ★ 窗口内点数上限**不需要**（点对数是窗口内点数的 C(n,2)，而窗口本身只有 1800 s：
//   10 s 一次轮询、每次都变化的上限是 180 个点 => 16110 个点对，一次算完是微秒级；
//   即使采样再密十倍也还是毫秒以下）。所以这里没有"最多取 N 个点"的常量。
//
// ★ 窗口长度**不在这里重复**：新口径与旧口径共用 3c 的 kRateWindowSeconds ——
//   两条链必须看同一段最近时间，否则"当前速度"会有两个含义。
//   曲线环的容量是存储自己的属性，在 curve_store.h（kCapacity = 一整天的轮询数）。
